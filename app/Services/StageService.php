...
class StageService
{
    ...
    public static function createReport($request, $id)
    {
        ...
        return ['documents' => $answer, 'description' => $result];
    }

    public static function getReports($id)
    {
        $reports = StageReports::query()->findOrFail($id);
        $document = Document::query()->findOrFail($reports->document_id);
        return ['description' => $reports, 'document' => $document];
    }
    ...
    {
        return StageReports::query()->where('status', '=', 'active')->get()->all();
    }
}
